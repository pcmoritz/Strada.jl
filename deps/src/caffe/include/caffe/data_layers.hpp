...
/**
 * @brief Provides data to the Net from memory. Difference to MemoryDataLayer is that there is no input for the labels.
 *
 * TODO(dox): thorough documentation for Forward and proto params.
 */
template <typename Dtype>
class PointerDataLayer : public BaseDataLayer<Dtype> {
 public:
  explicit PointerDataLayer(const LayerParameter& param)
      : BaseDataLayer<Dtype>(param), has_new_data_(false) {}
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "PointerData"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
